...
void solve(){
    ...
    for(ll dd=1;dd<=t;dd++){
		 ...
		 fflush(stdin);
		 ll lower=1,upper=n;
		 ll lower1=1,upper1=n;
		 ll i;
		 ll queries=300;
		 //Reference- https://www.geeksforgeeks.org/interactive-problems-in-competitive-programming/	 
		 for(i=0;i<queries;i++){
			if(i%2){
				if(lower<=upper){
				   //cout<<i<<" "<<"loop1"<<endl;
				   //cout<<"loop1"<<endl;
					...
					cin>>s;
					fflush(stdin);
					if(s=='L') upper=middle-1;
					else if(s=='G') lower=middle+1;
					else if(s=='E') exit(0);
					/*c++;
					/*else{
						// printf("%lld\n",n);
				...
				//cout<<"loop2"<<endl;
				//cout<<i<<" "<<"loop2"<<endl;
					ll middle1=lower1+(upper1-lower1)/2;
					printf("%lld\n",middle1);
					fflush(stdout);
					char s1;
					cin>>s1; //put scanf instead of cin and it will print 2 times, no idea why? XD
					fflush(stdin);
					if(s1=='L') upper1=middle1-1;
					else if(s1=='G') lower1=middle1+1;
					else if(s1=='E') exit(0);
					//c++;
					/*else{
						// printf("%lld\n",n);
 ...
 /*
  vector<ll>arr={5,10,10,5,15};
    ...
    for(ll i=1;i<=5;i++){
	 ...
	 if(solve(5,arr)!=bruteForce(5,arr)){
		 cout << "found a conflict!!" << "\n";
		 cout <<"my code gives "<<solve(5,arr)<< "\n";
		 cout <<"brute force gives "<<bruteForce(5,arr) << "\n";
