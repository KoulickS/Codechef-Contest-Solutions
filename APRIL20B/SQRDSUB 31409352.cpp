...
ll subarraysum(vector<ll>&v,ll k){
    ll sum=0;
    ll ret=0;
...
//Source- https://www.geeksforgeeks.org/highest-power-of-two-that-divides-a-given-number/
ll power2(ll n){
  return log2((n & (~(n-1))));
}
void solve(){
    ...
    for(ll d=0;d<tt;d++){
		...
		ll totalcount=subarraysum(v1,1);
		ll ans=totalsubarrays-totalcount;
		cout<<ans<<endl;
	}
}
int32_t main()
