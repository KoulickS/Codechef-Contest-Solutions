...
void solve(){
    ...
    for(ll d=0;d<tt;d++){
	    ...
	    ll i,j;
	    for(i=0;i<n;i++){
			for(j=0;j<k;j++){cin>>arr[i][j];}}
	    ll brr[10];
	    for(i=0;i<10;i++)brr[i]=0;
	    for(i=0;i<n;i++){
			for(j=0;j<10;j++){brr[j]=0;}
			for(j=0;j<k;j++)brr[arr[i][j]]++;
			//ll mx=*max_element(brr,brr+10);
			ll y=distance(brr,max_element(brr,brr+10));
			cout<<y<<" ";
		}	
		cout<<endl;
	}
