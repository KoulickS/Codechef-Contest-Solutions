...
/* Taken from https://codezen.codingninjas.in/practice/471/902/braille's-dilemma*/
ll dp[105][1<<12];
ll find_touch(ll pos, ll mask,vector<string>&v){
    if(!(mask&(mask-1))&&mask)
        return 0;
    if(pos==-1 || mask==0)
        return 100000;
    ...
    return dp[pos][mask]=min(find_touch(pos-1,newmask1,v)+find_touch(pos-1,newmask2,v)+touches,find_touch(pos-1,mask,v));

}
ll minimumtouch(ll n,vector<string>v){
    return find_touch(v[0].size()-1,(1<<n)-1,v);
}
void solve(){
    ll n;
    cin>>n;
