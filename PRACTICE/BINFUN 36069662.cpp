	     ...
	     if(n>908){
			 ...
			 for(i=0;i<iy;i++){
				...
				if(arr[i]==arr[n-1]){
				 ...
				 string s=decToBinary(arr[i]);
				 string s1=decToBinary(arr[f]);
				 string st1=s+s1;
				 string st2=s1+s;
				 ll z=binaryToDecimal(st1)-binaryToDecimal(st2);
				 string st11=s1+s;
				 string st22=s+s1;
				 ll z1=binaryToDecimal(st11)-binaryToDecimal(st22);
				 ll mx=maxi(z,z1);
				 v.pb(mx);
			 }
		 }
