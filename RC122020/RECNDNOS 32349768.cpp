...
void solve(){
    ...
    for(ll d1=0;d1<tt;d1++){
	   ...
	   for(i=0;i<n;i++){
		   cin>>arr[i];
		   if(vec.size()&&vec.back().first==arr[i])vec.back().second++;
		   else vec.pb({arr[i],1});
	   }
	   for(i=0;i<vec.size();i++){freq[vec[i].first]=freq[vec[i].first]+(vec[i].second+1)/2;}
	   ll ans=0;
	   ll ans2;
	   for(i=0;i<1005;i++){
		   if(freq[i]>ans){ans=freq[i];ans2=i;}}
	   cout<<ans2<<endl;   
	   /*ll c=0;
	   vector<pair<ll,ll>>v;
	   for(auto it:mp){
	   ...
	   sort(v.rbegin(),v.rend(),sortByVal);
	   for(i=1;i<n;i++){
		   if(v[0].first==arr[i] && arr[i-1]!=v[0].first){
			   c++;
		   }
