...
/* Taken from https://www.geeksforgeeks.org/program-check-given-year-leap-year/*/
...
/*Taken from https://www.geeksforgeeks.org/find-day-of-the-week-for-a-given-date/*/
...
void solve(){
    ...
    cin>>t;   
        ll i;
        for(i=1;i<=400;i++){
			if(!(checkYear(i)@0) && dayofweek(6,2,i)==5){
					v.pb(i);
			}
			else if(dayofweek(7,2,i)==5){
			      v.pb(i);	
			}
		}
		/*for(i=0;i<v.size();i++){
