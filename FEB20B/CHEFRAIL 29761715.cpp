...
void solve(){
    ...
    while(t--){
		...
		sort(v.begin(),v.end());for(i=0;i<M;i++){cin>>v1[i];mpp1[v1[i]]=1;}
		/*for(i=0;i<N;i++){
			cout<<v[i]<<" ";
		}*/
		ll ans=0;
		...
		for(i=0;i<(up(v.begin(),v.end(),0)-v.begin());i++)
		{if(v[i]==0){
				cc;}
		   for(j=(up(v.begin(),v.end(),0)-v.begin());j<N;j++)
		   {
			   ll q=v[j]*(-v[i]);if(q>=0){ll b=ss(q); if(pow(b,2)<q){cc;}ans+=mpp1[-b]; ans+=mpp1[b];}	
		   }}
	     if(mpp1[0]>0)
	     {
			 ans=ans+N*(M-1);}
	     ...
	     {
			 ans=ans+M*(N-1);}
		 for(i=0;i<(up(v1.begin(),v1.end(),0)-v1.begin());i++)
		  {if(v1[i]==0){
			   cc;}
		   for(j=(up(v1.begin(),v1.end(),0)-v1.begin());j<M;j++){
			   ll q=(v1[i]*v1[j])*(-1); if(q>=0){ll b=ss(q); if(pow(b,2)<q)cc;ans+=mpp[-b]; ans+=mpp[b];}	
		   }}
		 cout<<ans<<endl;
	 }
  }
