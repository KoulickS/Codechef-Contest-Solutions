...
void solve(){
    ...
    while(t--){
     ...
     // ll x=0,y=0,z=0,a=0;
      ...
      cout<<m1<<" "<<m2<<" "<<m3<<" "<<m4;
      vector<ll>v;
      v.pb(m1);v.pb(m2);v.pb(m3);v.pb(m4);
      ...
      ll qa;
         do{ string s="ABCD";
		   do{
			 ll price=0; for(i=0;i<sz;i++){ qa=arr[s[i]-'A'][i]; if(qa==0){
				   	price=price-100;
				 }
				 else{
				     price=price+cost[i]*qa;}
			 } 
			  maxx=max(maxx,price); }while(np(s.begin(),s.end())); }while(np(cost,cost+sz));
	  final_ans+=maxx; cout<<maxx<<endl;
    }
    cout<<final_ans<<endl;
}
int main()
