...
void solve(){
     ...
     for(ll dd=0;dd<t;dd++){
	     ll n,m,x,y;
	     cin>>n>>m>>x>>y;
	     if(n==1 and m==1){cout<<x<<endl;cc;}
	     ll prod=n*m;
	     x=mini(x,y); y=y-x;
	     y=mini(x,y);
	     ll ans=0;
	     ll ans1=0;
	     if(prod%2==0){ prod=prod/2; ans1=prod*x+prod*y;}
		 else{ans1=(prod-(prod/2))*y+(prod/2)*x; ans=(prod-prod/2)*x+(prod/2)*y;}
		 cout<<maxi(ans,ans1)<<endl;
     }
}
int32_t main()
{	
