...
bool localmaxima(ll a,ll b,ll c){
         if(max({a,b,c})==b)return true;
         return false;	
}
bool localminima(ll a,ll b,ll c){
...
void solve(){
    ...
    while(t--){
		...
		s1.ii(brr[0]);
		bool flag=false;
		if(arr[0]>=brr[0])flag=true;
		for(i=1;i<n;i++){
			if(brr[i]<=arr[i])flag=true;
			if(s1.find(arr[i])==s1.end())flag=true;
			s1.ii(arr[i]);s1.ii(brr[i]);
		}
		if(flag==false){
			cout<<"YES"<<endl;
		}
