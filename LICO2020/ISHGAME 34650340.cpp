...
ll *getCharCountArray(char *str) 
{ 
   ll *count = (ll *)calloc(sizeof(ll), NO_OF_CHARS); 
   ll i; 
   for (i = 0; *(str+i);  i++) 
      count[*(str+i)]++; 
   return count; 
...
{ 
  ll *count  = getCharCountArray(mask_str); 
  ll ip_ind  = 0, res_ind = 0; 
  while (*(str + ip_ind)) 
  { 
...
void solve(){
    //input
    char s[100003],s1[100003];
    cin>>s>>s1;
    cout<<removeDirtyChars(s,s1)<<endl;
}
int32_t main()
{	
     fio;
