...
ll kash2(string str,string s1) {
    ll a=kash(str);ll b=kash(s1);
    return a+b;
}
...
void solve(){
    ...
    while(tt--){
	    ...
	    cin>>x>>y;
	    ll i,j;
	    //string s=to_string(x); string s1=to_string(y);
	    if(x.length()<y.length())swap(x,y);
	    ll z=kash2(x,y);
	    for(i=0;x[i];i++){for(j=0;y[j];j++){swap(x[i],y[j]); z=max(z,kash2(x,y));swap(x[i],y[j]);
			}
		}cout<<z<<endl;
    }
}
